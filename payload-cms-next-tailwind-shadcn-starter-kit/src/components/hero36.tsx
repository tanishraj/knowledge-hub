import { ArrowRight, Blocks, Database, LayoutPanelTop, LucideIcon, Rocket } from 'lucide-react'
...
export interface Hero36Props {
  ...
  cardDescriptionProps?: (
    index: number,
    card: Hero36Card,
  ) => React.HTMLAttributes<HTMLParagraphElement> | undefined
  cardProps?: (
    index: number,
    card: Hero36Card,
...
const isHero36IconName = (value: string): value is Hero36IconName => value in hero36Icons
...
const Hero36 = (props: Props) => {
  ...
  return (
    <section className={cn('relative overflow-hidden bg-accent py-24 md:py-32', className)}>
      ...
      <div className="relative container flex flex-col items-center text-center">
        {badge && (
          <Badge
            {...badgeProps}
            variant="outline"
            className={cn('px-4 py-1.5 text-xs font-medium uppercase', badgeProps?.className)}
          >
            {badge.text}
          </Badge>
        )}
        <h1
          {...headingProps}
          className={cn(
            'my-4 max-w-3xl text-2xl font-bold text-pretty sm:text-4xl md:my-6 lg:text-5xl',
        ...
        {description && (
          <p
            {...descriptionProps}
            className={cn('max-w-2xl text-muted-foreground', descriptionProps?.className)}
          >
            {description}
          </p>
        )}
      ...
      <div className="container mt-12 grid gap-6 md:mt-16 md:grid-cols-3 md:gap-8">
        {cards.slice(0, 3).map((item, index) => {
          const currentCardProps = cardProps?.(index, item)
          const currentCardTitleProps = cardTitleProps?.(index, item)
          const currentCardDescriptionProps = cardDescriptionProps?.(index, item)
          ...
          return (
            <a
              ...
              className={cn(
                ...
                currentCardProps?.className,
              )}
            >
              {renderCardVisual(item)}
              <h3
                {...currentCardTitleProps}
                className={cn(
                  'mb-3 text-lg font-semibold md:text-xl',
                  currentCardTitleProps?.className,
                )}
              >
                {item.title}
              </h3>
              <p
                {...currentCardDescriptionProps}
                className={cn(
                  'mb-auto text-sm text-muted-foreground',
                  currentCardDescriptionProps?.className,
                )}
              >
                {item.description}
              </p>
